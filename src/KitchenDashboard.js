...
export default function KitchenDashboard() {
  ...
  const archiveOldOrders = async () => {
    const res = await fetch('https://https://californiaheartland-83cc4-default-rtdb.firebaseio.com/orders.json');
    ...
    for (const [id, entry] of Object.entries(data)) {
      ...
      const archiveCheck = await fetch(`https://https://californiaheartland-83cc4-default-rtdb.firebaseio.com/archive/${entryDateStr}/${id}.json`);
      const alreadyArchived = await archiveCheck.json();
      if (alreadyArchived) continue;

      await fetch(`https://https://californiaheartland-83cc4-default-rtdb.firebaseio.com/archive/${entryDateStr}/${id}.json`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...entry, Archived: true })
      });

      await fetch(`https://https://californiaheartland-83cc4-default-rtdb.firebaseio.com/orders/${id}.json`, {
        method: 'DELETE'
      });
  ...
  return (
    <div style={{ padding: '1rem', fontFamily: 'Arial' }}>
      ...
      <button
        onClick={async () => {
          if (!showArchived) {
            const res = await fetch('https://https://californiaheartland-83cc4-default-rtdb.firebaseio.com/archive.json');
            const data = await res.json();
            const allArchived = [];
